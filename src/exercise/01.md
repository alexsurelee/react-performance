...
For anything that you'd like to lazy load, but are confident will be needed, you
can prefetch this.

For webpack, this looks like:

```js
import(/* webpackPrefetch: true */ './some-module.js')
```

This means that even though there's lazy loading behaviour, this import will
wait until the rest of the page is loaded to be imported. This lets you display
your fallback UI as needed, but doesn't need to wait for any user interaction to
begin the lazy load, so that when the user does inevitably want to use this
code, it might already have preloaded.

Natively, this translates to:
